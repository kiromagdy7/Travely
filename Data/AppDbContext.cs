...
//    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
//#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
//        => optionsBuilder.UseSqlServer("Data Source = DESKTOP - Q70TV0H; Initial Catalog = TravelyDB; Integrated Security = True; Trust Server Certificate=True");
        ...
        modelBuilder.Entity<TblHotel>(entity =>
        {
            ...
            entity.HasMany(d => d.Amenities).WithMany(p => p.Hotels)
                .UsingEntity<Dictionary<string, object>>(
                    "TblHotelAmenity",
                    r => r.HasOne<LkpAmenity>().WithMany()
                        .HasForeignKey("AmenityId")
                        .HasConstraintName("FK_HotelAmenity_Amenity"),
                    l => l.HasOne<TblHotel>().WithMany()
                        .HasForeignKey("HotelId")
                        .HasConstraintName("FK_HotelAmenity_Hotel"),
                    j =>
                    {
                        j.HasKey("HotelId", "AmenityId").HasName("PK__tblHotel__8B6EFA74CCBE0F70");
                        j.ToTable("tblHotelAmenities");
